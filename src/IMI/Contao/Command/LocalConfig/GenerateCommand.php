...
class GenerateCommand extends AbstractContaoCommand
{
...
- The file "app/etc/local.xml.template" (bundles with Contao) must exist!
...
HELP;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $configFileTemplate = $this->_contaoRootFolder . '/app/etc/local.xml.template';
        if (!file_exists($configFile)) {
            $this->writeSection($output, 'Generate Contao local.xml');
            $this->askForArguments($input, $output);
            if (!file_exists($configFileTemplate)) {
                $output->writeln('<error>File ' . $this->_contaoRootFolder . '/app/etc/local.xml.template does not exist.</error>');
                return;
            }

            if (!is_writable($this->_contaoRootFolder . '/app/etc')) {
                $output->writeln('<error>Folder ' . $this->_contaoRootFolder . '/app/etc is not writeable</error>');
                return;
            }
            ...
            $replace = array(
                ...
                '{{db_init_statemants}}' => $this->_wrapCData('SET NAMES utf8'), // this is right -> contao has a little typo bug "statemants".
            ...
            $output->writeln('<info>local.xml file already exists in folder "' . $this->_contaoRootFolder . '/app/etc' . '"</info>');
        }
    }
    ...
    {
        $configFile = $this->_contaoRootFolder . '/app/etc/local.xml';
        return $configFile;
    }
