...
class InfoCommand extends AbstractContaoCommand
{
    ...
    protected function configure()
    {
        $this
            ->setName('sys:info')
            ->setDescription('Prints infos about the current contao system.')
            ->addOption(
                'format',
                null,
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('format') == null) {
            $this->writeSection($output, 'Contao System Information');
        }

        $this->initContao();

        $this->infos['Version'] = VERSION . '.' . BUILD;
    ...
    {
        $folders = array(
            $this->_contaoRootFolder . '/app/code/local/Mage',
            $this->_contaoRootFolder . '/app/code/local/Enterprise',
            $this->_contaoRootFolder . '/app/code/community/Mage',
            $this->_contaoRootFolder . '/app/code/community/Enterprise',
        );
    ...
    {
        $codePools = array(
            'core'      => $this->_contaoRootFolder . '/app/code/core/',
            'community' => $this->_contaoRootFolder . '/app/code/community/',
        );

        if (is_dir($this->_contaoRootFolder . '/app/code/local/')) {
            $codePools['local'] = $this->_contaoRootFolder . '/app/code/local/';
        }
