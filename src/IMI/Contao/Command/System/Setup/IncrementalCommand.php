 ...
 * @codeCoverageIgnore
 */
class IncrementalCommand extends AbstractContaoCommand
{
     ...
     * @param string $contaoExceptionOutput
     */
    protected function _processExceptionDuringUpdate(
        $e,
        $name,
        $setupResource,
        $contaoExceptionOutput
    )
    {
        $output = $this->_output;
        $output->writeln('<error>Contao encountered an error while running the following ' .
        ...
        $output->writeln(
            "<error>The Bad News:</error> Because Contao/MySQL can't run setup resources \n" .
        ...
        if ($contaoExceptionOutput) {
            $this->getHelper('dialog')->askAndValidate(
                $output,
                '<question>Press Enter to view raw Contao error text:</question> '
            );
            $output->writeln("Contao Exception Error Text:");
            echo $contaoExceptionOutput, "\n"; //echoing (vs. writeln) to avoid seg fault
        }
    }
    ...
    protected function _checkCacheSettings()
    {
        ...
        if ($allTypes['config'] !== '1') {
            ...
            $output->writeln('Please change your Contao settings at System -> Cache Management');
            $output->writeln('');
    ...
    {
        //bootstrap contao
        $this->detectContao($this->_output);
        if (!$this->initContao()) {
            return;
        }
