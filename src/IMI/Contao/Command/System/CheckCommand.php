...
class CheckCommand extends AbstractContaoCommand
{
    ...
    protected function configure()
    {
        $this
            ->setName('sys:check')
            ->setDescription('Checks Contao System')
            ->addOption(
                'format',
                null,
            ...
            if ($result->getMessage()) {
                switch ($result->getStatus()) {
                    ...
                    case Result::STATUS_ERROR:
                        $output->write('<error>' . \IMI\Util\Unicode\Charset::convertInteger(self::UNICODE_CROSS_CHAR) . '</error> ');
                        break;

                    default:
                    case Result::STATUS_OK:
                        $output->write('<info>' . \IMI\Util\Unicode\Charset::convertInteger(self::UNICODE_CHECKMARK_CHAR) . '</info> ');
                        break;
                }
