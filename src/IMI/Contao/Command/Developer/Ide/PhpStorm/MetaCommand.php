...
class MetaCommand extends AbstractContaoCommand
{
    /**
     * @var array
     */
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($this->initContao()) {
            if ($this->_contaoMajorVersion == self::MAGENTO_MAJOR_VERSION_1) {
                $classMaps = array();
                ...
                $output->write('Contao 2 is currently not supported');
            }
        }
    }
    ...
    protected function getResourceHelperMap()
    {
        ...
        if (($this->_contaoEnterprise && version_compare(\Mage::getVersion(), '1.11.2.0', '<='))
            || (!$this->_contaoEnterprise && version_compare(\Mage::getVersion(), '1.6.2.0', '<'))
        ) {
            return $classes;
        }
    ...
    {
        /**
         * Generate resource helper only for Contao >= EE 1.11 or CE 1.6
         */
...
PHP;
        if ($input->getOption('stdout')) {
            ...
            if (\file_put_contents($this->_contaoRootFolder . '/.phpstorm.meta.php', $map)) {
